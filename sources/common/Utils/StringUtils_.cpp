...
pchar SU::TimeMStoText(uint timeMS, char buffer[32])
{
    ...
    else
    {
        ...
        if (hours)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02uч", hours);
        }
        if (minutes)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02uм", minutes);
        }
        if (ms)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02u.%03uс", secs, ms);
        }
        else
        {
            std::sprintf(buffer + std::strlen(buffer), "%02uс", secs);
        }
    }
