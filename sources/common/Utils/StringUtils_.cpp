...
pchar SU::TimeMStoText(int timeMS, char buffer[32])
{
    ...
    if (timeMS < 1000)
    {
        std::sprintf(buffer, "%03dмс", timeMS);
    }
    else
    {
        if (days)
        {
            std::sprintf(buffer, "%dд", days);
        }
        if (hours)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02dч", hours);
        }
        if (minutes)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02dм", minutes);
        }
        if (ms)
        {
            std::sprintf(buffer + std::strlen(buffer), "%02d.%03dс", secs, ms);
        }
        else
        {
            std::sprintf(buffer + std::strlen(buffer), "%02dс", secs);
        }
    }
