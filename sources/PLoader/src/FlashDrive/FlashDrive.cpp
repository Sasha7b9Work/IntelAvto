...
bool FDrive::FileExist(const char *fileName)
{
    ...
    {
        ToLower(nameFile);
        if(std::strcmp(f, nameFile) == 0) //-V2513
        {
            return true;
        }
...
static bool GetNameFile(const char *fullPath, int numFile, char *nameFileOut, StructForReadDir *s)
{
    std::memcpy(reinterpret_cast<uint8 *>(s->nameDir), const_cast<char *>(fullPath), std::strlen(fullPath)); //-V2513 //-V2567
...
#define sizeSector (1 * 1024)
    ...
    while(size)
    {
        ...
        percentsUpdate = 1.0F - static_cast<float>(size) / (float)fullSize;
    }
