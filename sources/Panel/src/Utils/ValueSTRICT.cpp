...
// \todo –азобратьс€, почему не работает встроенна€ арифметика с 64 битами
#define MAX_UINT64 ((uint64)0xFFFFFFFFFFFFFFFF)     // ћаксимально возможное число уменьшено в 256 раз, потому что
...
void ValueSTRICT::AlignOrders(ValueSTRICT &v)
{
    if (v.order.value > order.value)
    {
        while (!LeadTo(v.order.value))
        {
            if (!v.DecreaseOrder())
            {
                return;             // ƒа, значение будет посчитано неправильно, но зато не будет зависани€
            }
        }
    }
